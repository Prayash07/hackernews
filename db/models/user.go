...
func (r *userR) GetLinks() LinkSlice {
	if r == nil {
		return nil
	}
	return r.Links
...
func (o *User) Links(mods ...qm.QueryMod) linkQuery {
...
func (userL) LoadLinks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`link`),
		qm.WhereIn(`link.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on link")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for link")
	}
...
func (o *User) AddLinksG(ctx context.Context, insert bool, related ...*Link) error {
...
// Appends related to o.R.Links.
// Sets related.R.User appropriately.
func (o *User) AddLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Link) error {
	var err error
...
// SetLinksG removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Links accordingly.
// Replaces o.R.Links with related.
// Sets related.R.User's Links accordingly.
// Uses the global database handle.
func (o *User) SetLinksG(ctx context.Context, insert bool, related ...*Link) error {
	return o.SetLinks(ctx, boil.GetContextDB(), insert, related...)
}

// SetLinks removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Links accordingly.
// Replaces o.R.Links with related.
// Sets related.R.User's Links accordingly.
func (o *User) SetLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Link) error {
	query := "update `link` set `user_id` = null where `user_id` = ?"
	...
	return o.AddLinks(ctx, exec, insert, related...)
}

// RemoveLinksG relationships from objects passed in.
// Removes related items from R.Links (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
// Uses the global database handle.
func (o *User) RemoveLinksG(ctx context.Context, related ...*Link) error {
	return o.RemoveLinks(ctx, boil.GetContextDB(), related...)
}

// RemoveLinks relationships from objects passed in.
// Removes related items from R.Links (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveLinks(ctx context.Context, exec boil.ContextExecutor, related ...*Link) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.Links {
			...
			ln := len(o.R.Links)
			if ln > 1 && i < ln-1 {
				o.R.Links[i] = o.R.Links[ln-1]
			}
			o.R.Links = o.R.Links[:ln-1]
			break
		}
	}
